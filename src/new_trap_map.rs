use crate::{
    trapezoidal_map::dag::Dag,
    trapezoidal_map::trap_map::{BoundingBox, Edge, Trapezoid},
    winding_number::{Point, Positioning},
    Mesh, PointLocator,
};
...
pub(crate) trait Nodal {
    fn get_trap(&self) -> &Trapezoid;
    fn get_trap_mut(&mut self) -> &mut Trapezoid;
    fn get_vertex(&self) -> usize;
    fn get_edge(&self) -> &Edge;
    fn next_node(&self, trap_map: &NewTrapMap, p: usize, xy: [f64; 2], slope: f64)
        -> Option<usize>;
...
impl Nodal for XNode {
    ...
    fn next_node(
        ...
        let idx = self.0;
        let vert = trap_map.vertices[idx];
        let left = !(p == idx || xy > vert);
        Some(if left { 0 } else { 1 })
    }
    ...
    fn get_edge(&self) -> &Edge {
        panic!("This is not a Y-node")
    }
}

struct YNode(Edge);

impl Nodal for YNode {
    fn get_trap(&self) -> &Trapezoid {
        panic!("This is not a Trapezoid")
    }
...
struct TrapNode(Trapezoid);

impl Nodal for TrapNode {
    fn get_trap(&self) -> &Trapezoid {
        &self.0
    }
...
pub struct NewTrapMap {
    dag: Dag<Box<dyn Nodal + Send>>,
...
impl NewTrapMap {
    ...
    pub(crate) fn add_bounding_box(&mut self, bbox: BoundingBox) {
        ...
        self.dag.add(Box::new(TrapNode(Trapezoid::new(
            nv,
            nv + 1,
            Edge {
                p: usize::MAX,
    ...
    pub fn from_mesh(mesh: Mesh) -> Self {
        ...
        let mut trap_map = NewTrapMap::init_with_mesh(mesh);
    ...
    pub(crate) fn init_with_mesh(mesh: Mesh) -> Self {
        ...
        let mut trap_map = NewTrapMap::new();
    ...
    pub(crate) fn add_edge(&mut self, edge: Edge) {
        ...
        for (i, &old_trap_idx) in trap_ids.iter().enumerate() {
            ...
            let (left_idx, right_idx, below_idx, above_idx) = if start_trap && end_trap {
                ...
                let below_idx = self.dag.add(Box::new(TrapNode(below)));
                let above_idx = self.dag.add(Box::new(TrapNode(above)));

                // Connect neighbors
                let left_idx = if have_left {
                    let left = Trapezoid::new(old.leftp, p, old.bottom, old.top);
                    let left_idx = self.dag.add(Box::new(TrapNode(left)));
                ...
                let right_idx = if have_right {
                    ...
                    let right_idx = self.dag.add(Box::new(TrapNode(right)));
            ...
            let si = if let Some(left_idx) = left_idx {
                ...
                self.dag
                    .entry(qi)
                    .and_modify(|node| *node = Box::new(XNode(q)));
                let si = self
                    .dag
                    .entry(qi)
                    .append_new(Box::new(YNode(edge)))
                ...
                self.dag
                    .entry(si)
                    .and_modify(|node| *node = Box::new(YNode(edge)));
                si
            };
    ...
    pub(crate) fn find_first_trapezoid(&self, edge: Edge) -> usize {
        ...
        loop {
            ...
            if let Some(child) = node.data.next_node(self, p, xy, slope) {
                d0 = node.children[child];
            } else {
                break;
            }
        }
    ...
    pub(crate) fn find_node(&self, point: &[f64; 2]) -> &Box<dyn Nodal + Send> {
        let mut node_id = 0;
        loop {
            let node = &self.dag.get(node_id).unwrap();
            match &node.data.get_type() {
                NodeType::Trap => break,
                NodeType::X => {
                    let idx = node.data.get_vertex();
                ...
                NodeType::Y => {
                    let Edge { p, q, .. } = node.data.get_edge();
    ...
    pub(crate) fn check(&self) {
        ...
        for node in self.dag.iter() {
            ...
            if node.children.is_empty() {
                assert!(
                    matches!(node.data.get_type(), NodeType::Trap),
    ...
    pub(crate) fn x_node_count(&self) -> usize {
        self.dag
            .iter()
            .filter(|&node| matches!(node.data.get_type(), NodeType::X))
            .count()
    }
    ...
    pub(crate) fn y_node_count(&self) -> usize {
        self.dag
            .iter()
            .filter(|&node| matches!(node.data.get_type(), NodeType::Y))
            .count()
    }
    ...
    pub(crate) fn trap_count(&self) -> usize {
        self.dag
            .iter()
            .filter(|&node| matches!(node.data.get_type(), NodeType::Trap))
            .count()
    }
    ...
    pub(crate) fn node_count(&self) -> (usize, usize, usize) {
        self.dag.iter().fold(
            ...
            |(mut x_count, mut y_count, mut trap_count), node| {
                match node.data.get_type() {
                    NodeType::X => x_count += 1,
                    NodeType::Y => y_count += 1,
                    NodeType::Trap => trap_count += 1,
                };
    ...
    pub fn depth_stats(&self) -> (f64, usize) {
        ...
        for (idx, node) in self.dag.iter().enumerate() {
            if matches!(node.data.get_type(), NodeType::Trap) {
                trap_count += 1;
                let depth = self
                    .dag
                    .depth(idx)
...
impl PointLocator for NewTrapMap {
    fn locate_one(&self, point: &[f64; 2]) -> Option<usize> {
        ...
        let face = match node.get_type() {
            NodeType::Trap => {
                let trap = node.get_trap();
            ...
            NodeType::X => {
                let idx = node.get_vertex();
                self.vertex_faces[idx]
            }
            NodeType::Y => node.get_edge().face,
        };
...
mod tests {
    ...
    fn locate_points_in_grid() {
        let mesh = Mesh::grid(0., 1., 0., 1., 2, 2).unwrap();
    ...
    fn trapezoidal_map_proptest() {
        ...
        let locator = NewTrapMap::from_mesh(mesh.clone()); // Clone the mesh to use it for verification with the winding number
